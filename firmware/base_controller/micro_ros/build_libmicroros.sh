...
#
# 산출물 micro_ros/libmicroros/ 는 .gitignore 대상이다. 레시피만 git에 두고 30 MB 짜리
# .a는 넣지 않는다. 필요하면 이 스크립트를 다시 돌린다.
set -euo pipefail
...
# ABI flag 대조. 애플리케이션과 라이브러리의 -mcpu/-mfpu/-mfloat-abi가 갈리면 링크는
# 통과하고 런타임에 깨진다. 그런 실패는 원인을 찾기가 아주 어렵다.
...
# 사용자 정의 메시지는 한 곳에서만 관리한다. 원본은 software/ros_pkgs/mr_msgs이고
# (호스트 노드가 그것을 빌드한다) 여기로는 빌드 직전에 복사만 한다. 두 벌을 각각
# 손으로 고치면 언젠가 갈린다. 그때 증상은 "agent가 디코드하지 못한다"로 나타나 원인을
# 엉뚱하게 펌웨어에서 찾게 된다.
