/**
  ...
  * @note    여기 있는 것은 차동구동 기하 하나뿐이다. 클램프 정책과 비정상 입력
  *          거절이 같이 들어 있는 이유는, 그 둘이 "무엇을 바퀴 지령으로 삼을
  *          것인가"라는 같은 질문의 일부이기 때문이다.
  ******************************************************************************
  */
...
typedef struct
{
  ...
  float   ticks_per_rev;
  int32_t max_tps;          /* 운용 상한. limits.md 2절 = VEL_TARGET_MAX_TPS */
} base_geometry_t;
...
/**
  ...
  *
  * @note   **상한을 넘으면 좌우를 같은 비율로 줄인다.** 바퀴마다 따로 잘라 내면
  *         회전 반경이 조용히 바뀐다 — 직진 지령이 상한에 걸리는 순간 한쪽만
  *         깎여서 로봇이 휘어 버리는 식이다. 비율을 지키면 속도만 느려진다.
  *
  *         NaN 거절이 이 자리에 있는 이유: `/cmd_vel`은 네트워크 밖에서 온 값이고,
  *         NaN이 한 번 들어오면 PI 적분기까지 오염돼 전원을 껐다 켜야 낫는다.
  *         숫자가 아닌 것은 제어 경로에 들이지 않는다.
  */
...
/**
  ...
  * @note   **double이다.** `position`은 랩어라운드 없는 누적값이라 몇 시간만
  *         굴러도 1e8 rad를 넘는데, float32는 그 지점에서 분해능이 수십 rad로
  *         떨어져 오도메트리가 쓰는 차분이 통째로 사라진다.
  */
