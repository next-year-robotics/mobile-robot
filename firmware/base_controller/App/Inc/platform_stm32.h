/**
  ...
  * @note    인터페이스에는 HAL 타입이 나오지 않는다. 애플리케이션은 이 헤더만 본다.
  *          HAL 레지스터 기록은 platform_stm32.c 안에만 존재한다.
  ******************************************************************************
  */
...
/**
  ...
  * @note   인터럽트 원을 하나도 켜지 않는다. scheduler가 서기 전에 task handle이 없는
  *         ISR이 뜨는 것을 막으려고 UART 수신 무장과 TIM6 기동은 각 소유 task의
  *         첫머리로 옮겼다.
...
/**
  ...
  * @note   허용된 call site는 platform_motor_kill() 내부뿐이다. 정상 경로의 TIM1
  *         소유자를 ControlTask 하나로 유지하기 위한 규칙이다. ISR-safe이며
  *         allocation/lock/blocking을 쓰지 않는다.
  */
...
/**
  * @brief  한 줄을 USART2로 내보낸다. 유한 타임아웃을 쓴다.
...
/**
  ...
  *         처음 무장한다. 이후 재무장도 같은 task와 RX/Error callback에서만 한다.
  */
