/**
  ...
  * @note    USART2 경로와 완전히 분리돼 있다. USART2는 레거시 ASCII와 ST-LINK 디버그
  *          콘솔로 남는다. 이 파일은 RPi5로 가는 GPIO UART만 소유한다.
  *
  *          수신은 circular DMA다. 인터럽트를 바이트마다 받지 않으므로 보드레이트를
  *          올려도 ISR 부하가 늘지 않는다. 스윕이 3 Mbaud까지 올라갈 수 있는 이유다.
  *          소비자는 NDTR로 생산 위치를 읽어 자기 tail부터 긁어 간다.
  *
  *          송신은 유한 타임아웃 블로킹이다. TX DMA는 생성만 돼 있고 여기서는 쓰지
  *          않는다.
  ******************************************************************************
  */
