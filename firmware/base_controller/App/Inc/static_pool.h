/**
  ...
  * @note    micro-ROS는 rcutils allocator를 통해 init과 재접속 경로에서 동적 할당을
  *          한다. 그 호출을 보낼 곳이 이 프로젝트에는 없다. FreeRTOS heap은
  *          `configSUPPORT_DYNAMIC_ALLOCATION == 0`이라 아예 없다. newlib malloc은
  *          `_sbrk`를 쓰는 데다 얼마나 썼는지 관측할 방법이 없다.
  *
  *          그래서 micro-ROS 전용 pool을 하나 두고 그 안에서만 놀게 한다. 상한은 링커
  *          시점에 박힌다. 최저 여유와 실패 횟수를 SWD로 볼 수 있다. 알고리즘은
  *          FreeRTOS heap_4와 같다.
  *
  *          소유자는 MicroRosTask 하나뿐이다. 내부에 잠금이 없으므로 다른 task에서
  *          부르면 free 리스트가 깨진다. 계측값도 소유 task가 복사한다.
  ******************************************************************************
  */
...
/**
  ...
  * @note   `bytes == 0`도 유일한 포인터를 돌려준다. malloc(0)의 관례다. rcl에는 길이
  *         0 sequence를 할당한 뒤 NULL 검사로 실패를 판정하는 자리가 있다.
  */
