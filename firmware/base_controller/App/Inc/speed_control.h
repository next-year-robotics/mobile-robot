/**
  ...
  *
  *          연산은 `float`다. 계수를 M3 회귀와 1:1로 적을 수 있고 Q 포맷 스케일이
  *          코드에 퍼지지 않는다. **와이어에는 float가 나가지 않는다** — 출력은
  *          정수 per-mille다.
  *
  *          M4.md 3.2 ~ 3.5의 식을 그대로 구현한다.
...
/**
  ...
  *         물리량이고, 전자를 상시 피드포워드에 넣으면 상수 바이어스가 전 운전점에
  *         걸려 적분기가 늘 그것을 상쇄하게 된다 (limits.md 2절).
  */
typedef struct
{
  float   kff;            /* ‰/tps — M3 duty-tps 회귀 기울기의 역수 */
...
/**
  ...
  * @note   목표가 0이면 PI를 돌리지 않고 duty 0 + 적분기 리셋이다. 관성으로
  *         굴러가는 바퀴에 역방향 duty를 걸지 않는다 — 정지는 코스팅이다.
  */
